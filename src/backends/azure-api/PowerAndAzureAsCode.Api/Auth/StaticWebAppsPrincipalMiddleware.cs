...
public sealed class StaticWebAppsPrincipalMiddleware(RequestDelegate next, IWebHostEnvironment environment)
{
    ...
    private static ClaimsPrincipal Parse(string encoded)
    {
        ...
        if (principal?.UserId is null) return new ClaimsPrincipal();
